...
<body class="sb-nav-fixed d-flex flex-column h-100">
    <h1 class="text-center mb-4">Edit Laporan Akhir PKM</h1>
    ...
    <div class="row justify-content-center mt-5">
        <div class="col-lg-8">
            <div class="card mb-4">
                <div class="card-body">
                    <div class="container">
                        <form action="{{ route('update.akhirpkm', $laporan_akhir->id) }}" method="POST"
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" id="JudulPKM" name="judul"
                                    value="{{ old('judul', $laporan_akhir->judul) }}" required>
                                    <option value="">Pilih...</option>
                                    @foreach($judulPKMs as $id => $judulPKM)
                                    <option value="{{ $judulPKM }}"
                                        {{ $laporan_akhir->judul == $judulPKM ? 'selected' : '' }}>
                                        {{ $judulPKM}}
                                    </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group mb-2">
                                ...
                                @if($laporan_akhir->file)
                                <p class="mt-2">Current File: {{ $laporan_akhir->file }}</p>
                                @endif
                            </div>
